...
def main() -> None:
    ...
    try:
        ...
        # Click on user Bob (triggers GET /api/messages?chatWith=bob)
        bob_item = wait.until(EC.element_to_be_clickable((By.XPATH, "//span[contains(@class, 'item-name') and text()='Bob Jones']")))
        bob_item.click()
        time.sleep(1)

        # Verify message area is active and showing messages
        msg_list = wait.until(EC.visibility_of_element_located((By.ID, "message-list")))
        assert msg_list.is_displayed(), "Messages log container should be loaded"
        print("GET Messages API via Selenium passed successfully.")
