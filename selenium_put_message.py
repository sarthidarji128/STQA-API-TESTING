...
def main() -> None:
    ...
    try:
        ...
        driver.execute_script("arguments[0].click();", caret_btn) # Use JS click for reliability in headless
        time.sleep(0.5)

        # Click Edit message
        edit_option = wait.until(EC.element_to_be_clickable((By.XPATH, "//div[contains(@class, 'msg-dropdown') and contains(@class, 'active')]//div[contains(@class, 'msg-dropdown-item') and contains(., 'Edit message')]")))
        edit_option.click()
        time.sleep(0.5)

        # Edit input
        edit_input = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, ".msg-edit-input")))
        edit_input.clear()
        edited_msg = f"Edited message {int(time.time())}"
        edit_input.send_keys(edited_msg)

        save_btn = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, ".msg-edit-btn.save")))
        save_btn.click()
        time.sleep(1)

        # Verify updated message
        chat_history = wait.until(EC.visibility_of_element_located((By.ID, "message-list"))).text
        assert edited_msg in chat_history, "Edited text should be visible"
        assert orig_msg not in chat_history, "Original text should be gone"
        print("PUT Message API via Selenium passed successfully.")
